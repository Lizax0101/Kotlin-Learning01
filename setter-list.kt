...
class Classificacao {
    var pontos: MutableList<Int> = mutableListOf()
        //a lista começa vazia (mutableListOf() cria uma lista mutável sem elementos)
        set(value) {//value → é a lista desordenada que tentei  atribuir
            // sempre guarda a lista em ordem decrescente
...
fun main() {
    println("Ranking por pontuação, formato decrescente.")
    val ranking = Classificacao()
    ranking.pontos = mutableListOf(900, 750, 1, 0)// uma lista desordenada

    /// ranking.pontos = 50
    println(ranking.pontos)

    /**
     * O setter entra em ação e organiza automaticamente.
     * Quando imprimimos println(ranking.pontuacoes), o getter retorna o field, que agora contém a lista já tratada
     */
}
